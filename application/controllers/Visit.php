...
//            }
            ...
            switch ($val['status']) {
                case 'Approved':
                    $current_status = '<span class="btn btn-success">Approved</span>';
                    break;
                case 'Rejected':
                    $current_status = '<span class="btn btn-danger">Rejected</span>';
                    break;
                case 'Blacklisted':
...
//        if($session_data['login_user_type']=="SUPER"){
//        $links = "";
//         $links = "<a href='$url'><i class='fa fa-pencil'></i></a>";
         $links = "<a href='javascript:void(0)' rel='$track_url' onclick='open_track(rel)' title='Open track'>
             <i class='fa fa-book track_color_box' ></i>
         </a>
         <a href='javascript:void(0)' rel='$visit_info_url' onclick='open_visit_info(rel)' title='Open visit info'>
             <i class='fa fa-eye track_color_box' ></i>
...
//        }
    ...
    {
        $query = "SELECT * FROM visit v INNER JOIN visitor_profile vf ON (vf.visitor_id = v.visit_visitor_id_fk) 
                    WHERE visit_id = '" . $id . "'";
        ...
        if ($get_visitor_Detail) {
            ...
            $insert_data = array('visit_idfk' => $id,
                'visitor_idfk' => $visitor_id,
                'visitor_cnic' => $cnic,
